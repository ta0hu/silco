...
class SSD(nn.Module):
    def __init__(
        ...
        from models.ssd.feature_reweighting_module import GNN_nl

        self.gcns = nn.ModuleList(
            [
                GNN_nl(
                    input_features=512, output_dim=1, gcn_layer=gcn_layer, nf=96
                ),  # 512
                GNN_nl(
                    input_features=1024,
                    output_dim=1,
                ...
                GNN_nl(
                    input_features=256, output_dim=1, gcn_layer=gcn_layer, nf=96
                ),  # 256
                GNN_nl(
                    input_features=256, output_dim=1, gcn_layer=gcn_layer, nf=96
                ),  # 256
        ...
        from models.ssd.spatial_similarity_module import _NonLocalBlockND
        ...
        self.conv_fusions = nn.ModuleList(
            [
    ...
    def forward(self, support_images, query_image, is_train):
        ...
        def fusion(source_feat):
            ...
            for context_level in range(len(feat_query)):
                ...
                b, c, w, h = _feat_query.shape
                bs, c, w, h = _feat_support.shape
                shot_num = int(bs / b)
                origin_channel_feat = _feat_support  # [B*S,C,W,H]
                gap = nn.AvgPool2d(_feat_support.size()[2:])  # BSxCx1x1

                # channel attention feature
                bs, c, feat_w, feat_h = origin_channel_feat.shape
                origin_channel_feat = origin_channel_feat.view(
                    b, shot_num, c, w, h
                )

                fused_list = []
                for sss in range(shot_num):
                    nled_query = self.nls[context_level](
                        _feat_query, origin_channel_feat[:, sss, :, :, :]
                    ).view(b, c, w, h)
                    fused_list.append(nled_query.view(b, 1, c, w, h))
                fused_feat = torch.cat(fused_list, 1)

                gcn_feat = torch.squeeze(
                    gap(origin_channel_feat.view(-1, c, w, h))
                )  # BSxC
                gcn_feat = gcn_feat.view(-1, shot_num, c)  # BxSxC
                gcn_feat = self.gcns[context_level](gcn_feat)
                gcn_feat = torch.sigmoid(
                    gcn_feat.view(-1, shot_num, 1, 1, 1)
                )  # [B, S, 1]=>[B, S, C, 1, 1]
                fused_feat = torch.sum(
                    fused_feat * gcn_feat, dim=1, keepdim=False
                )
                fused_feat = self.relu(
                    self.conv_fusions[context_level](fused_feat)
                )
