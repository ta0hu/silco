...
from cl_utils.pytorchgo_util import model_summary
...
class FCN(nn.Module):
    ...
    def forward(self, support_images, query_image, is_train=True):
        # x = torch.cat((support_images, query_image.unsqueeze(dim=1)), 1)
        x = torch.cat((query_image.unsqueeze(dim=1),), 1)
...
def VGG16_LargeFoV(class_num, image_size, pretrained=False, **kwargs):
    ...
    vgg_input = nn.Sequential(  # Sequential,
        ...
        Interpolate(size=image_size, mode="bilinear"),
    )
    model = FCN(vgg_input, **kwargs)

    if pretrained:
        saved_state_dict = torch.load("deeplabv1_init_model.pth")
        # saved_state_dict = model_zoo.load_url('deeplabv1_init_model.pth')
        ...
        for weight_key in saved_state_dict:
            ...
            if "38" in weight_key:
                continue
            new_params["features.{}".format(weight_key)] = saved_state_dict[
                weight_key
            ]
            logger.info(
